...
//hosten
@property (nonatomic, assign) CGFloat xMaxCount;
...
- (void)mappingWithLineId:(NSString *)lineID lineColor:(UIColor *)color{
    //hosten
    _xMaxCount = [UIScreen mainScreen].bounds.size.width/_xScaleMarkLEN;
...
//        axisViewWidth = self.xScaleMarkLEN * (self.xMarkTitles.count - 1) + HORIZON_YMARKLAB_YAXISLINE + YMARKLAB_WIDTH + XMARKLAB_WIDTH / 2.0;
        axisViewWidth = self.xScaleMarkLEN * (_xMaxCount - 1) + HORIZON_YMARKLAB_YAXISLINE + YMARKLAB_WIDTH + XMARKLAB_WIDTH / 2.0;
    }
    
    self.xAxis_L = self.xScaleMarkLEN * (_xMaxCount - 1);

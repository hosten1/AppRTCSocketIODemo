 ...
 *  terminates.
 */
- (void)rtcAudioSessionMediaServerTerminated:(RTCAudioSession *)session;

/** Called on a system notification thread when AVAudioSession media server
 *  restarts.
 */
- (void)rtcAudioSessionMediaServerReset:(RTCAudioSession *)session;

// TODO(tkchin): Maybe handle SilenceSecondaryAudioHintNotification.

- (void)rtcAudioSession:(RTCAudioSession *)session didChangeCanPlayOrRecord:(BOOL)canPlayOrRecord;

/** Called on a WebRTC thread when the audio device is notified to begin
 *  playback or recording.
 */
- (void)rtcAudioSessionDidStartPlayOrRecord:(RTCAudioSession *)session;

/** Called on a WebRTC thread when the audio device is notified to stop
 *  playback or recording.
 */
- (void)rtcAudioSessionDidStopPlayOrRecord:(RTCAudioSession *)session;

/** Called when the AVAudioSession output volume value changes. */
- (void)rtcAudioSession:(RTCAudioSession *)audioSession didChangeOutputVolume:(float)outputVolume;
...
@end
@protocol RTCAudioSessionDataSource <NSObject>

@optional

///  说明要 重新初始化音频
/// @param audioSession  会话管理
/// @param active  描述
/// @param error 错误信息 如果有
    - (RTCAudioSessionConfiguration*)rtcAudioSession:(RTCAudioSession *)audioSession
        needConfigureWebRTCSession:(BOOL)active
                    error:(NSError **)error;

/// 取消配置
/// @param audioSession 会话管理
/// @param outError  错误信息 如果有
- (BOOL)rtcAudioSession:(RTCAudioSession *)audioSession
unconfigureWebRTCSession:(NSError **)outError;
...
- (void)rtcAudioSessionDidActivate:(AVAudioSession *)session;

/** Called when the audio session is deactivated outside of the app by iOS. */
- (void)rtcAudioSessionDidDeactivate:(AVAudioSession *)session;
...
@property(nonatomic, weak) id<RTCAudioSessionDataSource> dataSource;
/// 在调用config的时候标记成true   结束的时候必须调用设置为NO
//@property(nonatomic, assign) BOOL hasConfiguredSession;
...
// lym bengin
- (void)InterruptionNotificationBeginWithReason:(NSString*)reason;
- (void)InterruptionNotificationEndWithReason:(NSString*)reason;
// lym end
@end

...
- (void)addOtherAppAudioDataWithAppData:(const int16_t*) audioSamples audioDataSize:(const size_t) audioDataSize nSamples: (const size_t) nSamples nChannels:(const size_t) nChannels nBitsPerChannel:(const size_t)nBitsPerChannel totalDelayMS: (const uint32_t) totalDelayMS;
- (void)startOtherAppAudioDataWithStart:(BOOL) startOtherAppData nSamples:(const size_t) nSamples nChannels:(const size_t) nChannels;
// lym 共享音频支持接口 end

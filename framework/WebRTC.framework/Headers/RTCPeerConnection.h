...
/*lym 修改*/
typedef NS_ENUM(NSInteger, RTCCustomizeValueType) {
  RTCValueCrypto = 1,
};
typedef NS_ENUM(NSInteger, RTCCustomizeSubType) {
  RTCValueSendCipherSelected = 1,
  RTCValueRecvCipherSelected,
  RTCValueCipherSuite,
};
/*lym 修改*/
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didProcessedRecordedDataReady:(NSData*)data
sampleRate:(size_t) sampleRate
nChannels: (const uint32_t) nChannels
samplesPerChannal: (const uint32_t) samplesPerChannal
durationMs: (const uint32_t) durationMs isVad:(BOOL) isVad;

- (void)peerConnection:(RTCPeerConnection *)peerConnection didProcessedPlayDataReady:(NSData*)data
sampleRate:(size_t) sampleRate
nChannels: (const uint32_t) nChannels
samplesPerChannal: (const uint32_t) samplesPerChannal
durationMs: (const uint32_t) durationMs isVad:(BOOL) isVad;
...
- (NSString*) getWebRtcValueWithType:(RTCCustomizeValueType) type subType:(RTCCustomizeSubType)subType;

//                lym
- (int32_t)recordedDataIsReadyWithAudioData:(const int16_t*) audio_data
                           number_of_frames:(const size_t) number_of_frame
                         number_of_channels:(const size_t) number_of_channels
                                sample_rate:(const uint32_t) sample_rate
                   audio_delay_milliseconds:(const uint32_t)audio_delay_milliseconds
                                      scale:(const float) scale;

- (void)updateExportAudioDataConfigWithEnableSendTranlateData:(BOOL) enableSendTranslateData
enableSourceData:(BOOL)  enableSendSourceData minimumSegmentDurations:(float) minSegmentDuration_s maximumSegmentDurations:(float) maximumSegmentDuration_s;
